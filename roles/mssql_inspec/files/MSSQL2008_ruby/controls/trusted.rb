# MSSQL 2008 InSpec Controls - CIS Benchmark
# CIS Microsoft SQL Server 2008 Benchmark
# Version: 3.0.0
# Last Updated: 2026-02-16
#
# This profile implements CIS Microsoft SQL Server 2008 Benchmark controls
# for enterprise compliance scanning.
#
# Control ID Format: mssql-2008-X.XX
# Tags: cis (control number), cis_level (1 or 2), severity (critical/high/medium/low)
#
# Note: MSSQL 2008 is end-of-life. Controls adapted for legacy compliance needs.
...
control 'mssql-2008-4.02' do
  ...
  describe sql.query("SELECT CASE WHEN COUNT(*) = 0 THEN 'COMPLIANT' ELSE 'NOT COMPLIANT' END AS results FROM sys.database_principals dp LEFT JOIN sys.server_principals sp ON dp.sid = sp.sid WHERE dp.type IN ('S', 'U') AND sp.sid IS NULL AND dp.name NOT IN ('dbo', 'guest', 'INFORMATION_SCHEMA', 'sys', 'MS_DataCollectorInternalUser')") do
    its('rows.first.results') { should eq 'COMPLIANT' }
  end
end

control 'mssql-2008-4.03' do
  impact 1.0
...
control 'mssql-2008-5.04' do
  impact 1.0
  title 'Ensure C2 Audit Mode is Configured'
  desc 'C2 audit mode provides comprehensive auditing for compliance (SQL 2008 specific).'

  tag cis: '5.4'
  tag cis_level: 2
  tag severity: 'critical'

  describe sql.query("SELECT CASE WHEN value_in_use = 1 THEN 'COMPLIANT' ELSE 'NOT COMPLIANT' END AS results FROM sys.configurations WHERE name = 'c2 audit mode'") do
    its('rows.first.results') { should eq 'COMPLIANT' }
  end
end
